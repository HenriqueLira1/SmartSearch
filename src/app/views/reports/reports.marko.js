...
function render(input, out, __component, component, state) {
  ...
  marko_forEach(data.reports, function(report) {
    var keyscope__30 = "[" + ((for__29++) + "]");

    out.w("<tr><td>" +
      marko_escapeXml(report.name) +
      "</td><td>" +
      marko_escapeXml(report.cpf) +
      "</td><td>" +
      marko_escapeXml(report.createdAt) +
      "</td><td style=\"display: inline-flex;\">");

    if (report.processing) {
      out.w("<div class=\"alert alert-warning\">'Processando relatório!'</div>");
    } else if (report.fatalApiError) {
      out.w("<div class=\"alert alert-danger\">'Falha ao gerar relatório!'</div>");
    } else if (report.apiError) {
      out.w("<a" +
        marko_attr("href", `/report/${report.id}`) +
        " class=\"btn btn-primary\" style=\"height: fit-content;\"><i class=\"fa fa-eye\"></i></a><div class=\"alert alert-warning\" style=\"margin-left: 10px;\">'Alguns dados não foram gravados!'</div>");
    } else {
      out.w("<a" +
        marko_attr("href", `/report/${report.id}`) +
